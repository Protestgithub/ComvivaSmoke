...
import ChanneltoChannel from '../PageObjects/ChanneltoChannel/ChanneltoChannel';
import TransactionCorrection from '../pageObjects/TransactionCorrection';
//-------------------------Object Declaration----------------------------------------------------------
...
var BAlogin='cypress/fixtures/userData/BusinessAdminLogin.json'
...
var TransactionCorrectionData = 'cypress/fixtures/userData/TransactionCorrectionData.json'
Cypress.Commands.add('TransactionCorrectionIDWriteData', () => {
    tranCorrPage.getsuccessmessage().then((al => {
       
        let q = al.text()
        cy.log(q)
        let a = q.split('approved')
        let b = a[1].trim()
        cy.log(b)
        let c = b.split(',')
        let d = c[0].trim()
        ...
        cy.writeFile(TransactionCorrectionData,{TransactionID:d})
        
    }))
})

Cypress.Commands.add('TransactionCorrectionIDWriteData1', () => {
    tranCorrPage.getsuccessmessage().then((al => {
        let q = al.text()
        cy.log(q)
        let a = q.split('approved')
        ...
        let d = c[0].trim()
        cy.log(d)
        cy.readFile(TransactionCorrectionData).then((data => {
            data.TransactionID1 = d
            cy.writeFile(TransactionCorrectionData, data)
...
Cypress.Commands.add('O2CTransactionWriteData', () => {
    O2CTransferInitiatePage.getSuccessMsg().then((al => {
        ...
        cy.log(b)
            ...
            cy.writeFile(TransactionFile, {TransactionID:b})
...
Cypress.Commands.add('checkAPI', (API) => {
    ...
    cy.wait('@all', { timeout: 70000 }).then(inter => {
        ...
        else if (inter.response.status == 200) {
            cy.log(' status new  loop entered now')
            cy.log(JSON.stringify(inter.response.statusCode))
            console.log(JSON.stringify(inter.response.statusCode))
            cy.log('  status new loop exit now')
        }

        else if (inter.response.status == 'PAUSED') {
            cy.log(' SUCCESS MESSAGE new  loop entered now')
            cy.log('  SUCCESS MESSAGE new loop exit now')
        }
        else if (inter.response.status == "SUCCEEDED") {
            cy.log(' SUCCEEDED MESSAGE new  loop entered now')
            cy.log('  SUCCESS MESSAGE new loop exit now')
        }
        else {
            cy.log('loop nvr entered')
