...
And('User click on approve', function () {
  ...
  approvalPage.getApproveConfirmationMessage().should('contain.text', this.data2.confirmationMessage.editUser)
})
...
Then('Verify Sucess Message', function () {
  cy.waitUntil(() => {
    return cy.iframe().find('p.text-center').should('be.visible')
  })
