    ...
    stages {
        ...
        stage('Sys'){
            steps{
                ...
                bat "./node_modules/.bin/cypress.cmd install --force "
                bat "npm run cy:parallel --env Adminurl=http://172.25.48.237 ,apiBaseURL=http://172.25.48.237:3133 --browser ${BROWSER} --spec ${SPEC}  "
        ...
        stage('Testing') {
            ...
            steps {
                ...
                bat "npm run cy:parallel:many --env Adminurl=http://172.25.48.237 ,apiBaseURL=http://172.25.48.237:3133 --browser ${BROWSER} --spec ${SPEC}  "
            }
            }
            stage('Test2') {
            steps {
                bat "npm run cy:parallel:many1 --env Adminurl=http://172.25.48.237 ,apiBaseURL=http://172.25.48.237:3133 --browser ${BROWSER} --spec ${SPEC}  "
            }
        }
        stage('Test3') {
            steps {
                bat "npm run cy:parallel:many2 --env Adminurl=http://172.25.48.237 ,apiBaseURL=http://172.25.48.237:3133 --browser ${BROWSER} --spec ${SPEC}  "
            }
        }
            }
        }
